...
RED.editor = (function() {
    ...
    function validateNode(node) {
        // console.log('validateNode: %s:%s', node.id, node.name);
        ...
        // console.log('validateNode: %s:%s-%s', node.id, node.name, node.valid);
        return node.valid;
    }
    ...
    function validateNodeProperty(node,definition,property,value) {
        ...
        if (valid && definition[property].type && RED.nodes.getType(definition[property].type) && !("validate" in definition[property])) {
            if (!value || value == "_ADD_") {
                ...
                // console.log('Config Node ? %s %s', value, configNode ? configNode.id : 'notConfigNode');
                valid = (configNode !== null && (configNode.valid == null || configNode.valid));
                if (configNode!==null){
                    // console.log('ConfigNodeValid %s', valid);
                }
    ...
    function prepareConfigNodeSelect(node,property,type,prefix) {
        ...
        $('#'+prefix+'-lookup-'+property).click(function(e) {
            console.log('Clicked (1) on Config Node %s-%s-%s', property, type, prefix);
            var ninput = select.find(":selected");
            var inputVal = ninput.attr('editContext') ? ninput.attr('editContext') : '';
            console.log('inputVal',inputVal);
            showEditConfigNodeDialog(property, type, ninput.val(), prefix, inputVal);
            e.preventDefault();
        });
        var label = "";
    ...
    function prepareConfigNodeButton(node,property,type,prefix) {
        ...
        button.click(function(e) {
            console.log('Clicked (2) on Config Node %s-%s-%s', property, type, prefix);
            var inputVal = input.attr('editContext') ? input.attr('editContext') : '';
            console.log('inputVal', inputVal);
            showEditConfigNodeDialog(property, type, input.val() || "_ADD_", prefix, inputVal);
            e.preventDefault();
        });
    }
     ...
     */
    function showEditConfigNodeDialog(name,type,id,prefix, data) {
        var adding = (id == "_ADD_");
        var node_def = RED.nodes.getType(type);
        ...
        editing_config_node._def.callerContext = data;
