...
(function($) {
    var allOptions = {
        ...
        flow: {value:"flow",label:"flow.",validate:RED.utils.validatePropertyExpression, autocomplete: true, filter: "flow"},
        global: {value:"global",label:"global.",validate:RED.utils.validatePropertyExpression, autocomplete: true, filter: "global"},
        ...
        variable: {
          ...
          label: "variable",
          autocomplete: true,
          autocompleteOptionsPromise: $.getJSON("projects/variables")
        },
        json: {
            value:"json",
            label:"JSON",
        ...
        type: function(type) {
            if (!arguments.length) {
                ...
                if (opt && this.propertyType !== type) {
                    ...
                    if (opt.autocomplete) {
                      $.getJSON("projects/variables")
                      .then( function(data) {
                        if (opt.filter == "flow") {
                          that.element.autocomplete({
                            source: data.variables.filter( function(variable) { return variable.flowID == window.location.hash.substring(6) }).map( function(variable) { return variable.name }),
                            minLength: 2
                          });
                        }
                        else if (opt.filter == "global") {
                          that.element.autocomplete({
                            source: data.variables.filter( function(variable) { return variable.flowID == null }).map( function(variable) { return variable.name }),
                            minLength: 2
                          });
                        }
                      });
